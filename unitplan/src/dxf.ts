// ---------------------------------------------------------------------------
// Deterministic ASCII DXF R12 writer for unit plans. Adapted from the site
// kernel's writer (kernel/src/dxf.ts); geometry is model-space metres at 1:1.
// Group-code 999 markers allow the acceptance tests to round-trip every path
// without relying on entity order.
// ---------------------------------------------------------------------------
import {
  UNIT_LAYERS,
...
function header(model: UnitDrawingModel): string {
  const { bounds } = model
  return (
    group(0, 'SECTION')
    + group(2, 'HEADER')
...
function layerRecord(layer: UnitLayerStyle): string {
  return (
    group(0, 'LAYER')
    + group(2, layer.name)
    + group(70, 0)
...
function tables(model: UnitDrawingModel): string {
  ...
  output += group(0, 'TABLE') + group(2, 'LAYER') + group(70, UNIT_LAYERS.length)
  for (const layer of UNIT_LAYERS) output += layerRecord(layer)
  output += group(0, 'ENDTAB')
...
function pathEntity(path: UnitDrawingPath): string {
  let output = group(999, `URBANOS_PATH ${encodeURIComponent(path.id)}`)
  output += (
    group(0, 'POLYLINE')
    + group(8, path.layer)
    + group(66, 1)
...
function textEntity(text: UnitDrawingText, scaleDenominator: number): string {
...
export function unitDrawingToDxf(model: UnitDrawingModel): string {
  ...
  output += group(999, `URBANOS_SOURCE ${asciiFold(model.sourceNote)}`)
  for (const note of model.assumptionNotes) {
    output += group(999, `URBANOS_ASSUMPTION ${asciiFold(note)}`)
  }
  output += group(999, `URBANOS_DIGEST ${model.digest}`)

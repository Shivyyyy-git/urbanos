...
function real(value: number): string {
  if (!Number.isFinite(value)) {
    fail('E_GEOMETRY_PARITY', `DXF writer received a non-finite number: ${String(value)}`)
  }
...
function layerRecord(layer: DemoLayerStyle): string {
  return (
    group(0, 'LAYER')
    + group(2, layer.name)
    + group(70, 0)
...
function tables(model: DemoDrawingModel): string {
  ...
  output += group(0, 'TABLE') + group(2, 'LAYER') + group(70, DEMO_LAYERS.length)
  for (const layer of DEMO_LAYERS) output += layerRecord(layer)
  output += group(0, 'ENDTAB')
...
function pathEntity(path: DemoDrawingPath): string {
...
function textEntity(text: DemoDrawingText, scaleDenominator: number): string {
...
export function communityDrawingToDxf(model: DemoDrawingModel): string {
  ...
  // Visible stamp + title lines in the title area below the drawing (model
  // space, south of the dimension band).
  const { bounds } = model
  const baseY = bounds.minY - 6
  const stampText: DemoDrawingText = {
    id: 'anno.stamp-title-area',
    layer: 'ANNOTATION',
    at: [bounds.minX + 4, baseY],
    heightMm: 4,
    text: model.stamp,
    rotationDegrees: 0,
    align: 'left',
  ...
  output += textEntity(stampText, model.scaleDenominator)
  model.titleLines.forEach((line, index) => {
    output += textEntity(
      {
        id: `anno.title-line-${index}`,
        layer: 'ANNOTATION',
        at: [bounds.minX + 4, baseY - 5 - index * 4],
        heightMm: 2.4,
        text: line,
        rotationDegrees: 0,
        align: 'left',

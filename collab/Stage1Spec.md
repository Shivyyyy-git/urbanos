...
**Status:** DRAFT v1.1 — D3/D4 ruled by Shivam 2026-08-16; ratification now blocked only on D1/D2 (Mannu) + the M-S register. Spec only, no code.
...
- **One plot type:** residential plotted, builder-floor intent (the stilt+N scenario). No commercial, no group housing, no amalgamated plots.
...
RuleEntry {
  ...
  source                          — document ref + scan/photo, page
...
**Fail-closed on data:** a required slot with no loaded entry for the slice ⇒ output blocked, missing slot named. A loaded but unverified entry ⇒ output allowed but stamp-limited (§7). No engine default exists for any slot.
...
1. **Legal Envelope Report** — PDF + machine-readable JSON:
   ...
   - declared inputs and assumptions printed in full; rulebook version digest pinned
...
- Any cited entry `unverified`, or any declared-but-unconfirmed assumption ⇒ **"Research Draft — Not for Construction"**, forced.
